...
Funcion para actualizar mas de un periodo
...
```{r}
lm(datos_ej, formula = y ~ income.level + price.index)
```
...
```{r}
pronos_dlm <- pronosticos(datos_ej, m20, C20, G20, W20, V20)
```

```{r}
df_pronos <- data.frame("fecha" = datos_ej %>% row.names(), "y_real" = datos_ej$y, 
...
```{r}
df_params_pronos <- data.frame(reduce(pronos_dlm$at_k, cbind) %>% t(), fecha = df_pronos$fecha)  %>% 
  rename(Intercepto = X1, Ingreso = X2, Precio = X3) %>% 
  pivot_longer(names_to = "parametro", values_to = "valor", 
               cols = c(Intercepto, Ingreso, Precio))
```
...
```{r}
ggplot(df_params_pronos, aes(x=fecha, y = valor)) +
  geom_line() +
  facet_wrap(~parametro, nrow = 3, scales = "free") +
  theme_bw()  +
  ylab("Valor") +
