...
F[1, 2, ] <- fix ## Nx1; predictor variable
A <- matrix(0) ## 1x1; scalar = 0
V <- matrix("v") ## 1x1; scalar = r # Es Vt

## only need starting values for regr parameters
inits_list <- list(x0 = matrix(c(350, -5), nrow = m))
...
df_params <- data.frame('fecha' = datos_efectivo$fecha, 'intercept' = theta_priori[1,], 
                        'fix' = theta_priori[2,]) %>% 
  pivot_longer(cols = c(intercept, fix), names_to = 'parametro', values_to = 'valor')
